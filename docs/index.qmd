...
# Binomial Models {background-color="#D9DBDB"}
...
performance::binned_residuals(beetle_glm)
...
# Data Visualisation {background-color="#D9DBDB"}
...
```{r,  out.width = "40%", fig.cap ="Distribution of Body Mass (g) across three species of Archipelago penguins. Boxplots display the median (central line), interquartile range (IQR, box),the whiskers extend to the smallest and largest values within 1.5 times the IQR. The violin (shaded area):represents the kernel density estimation of the data distribution. Individual dots represent individual observations. "}
...
penguins |> 
  ...
  ggplot(aes(x = species,
             y = count)) +
  geom_segment(aes(x = species, xend = species, y = 0, yend = count), size = 1.5) +
...
```{r, fig.width = 6,  out.width = "40%", fig.cap ="Relationship Between Bill length(mm) and Bill depth (mm). Each point represents an individual data observation. (if applicable): A fitted [linear/nonlinear] regression line is included to indicate the overall trend in the data. Shaded regions (if present) represent the confidence interval of the fit. Points are colored and shaped according to species."}
...
## Figures vs Data visualisations
...
ggplot(penguins, aes(x= bill_length_mm, 
  ...
  theme_classic()+
    labs(x="Bill length (mm)",
         y="Bill depth (mm)",
    title = "There is a positive relationship between Bill length and Bill depth",
    subtitle = "for all <span style = 'color:#FF8C00'>Adelie</span>, <span style ='color:#159090'>Gentoo</span>, and <span style = 'color:#A034F0'>Chinstrap</span> penguin species")+
  theme(legend.position = "bottom",
...
```{r, echo=FALSE,  out.width="70%"}
knitr::include_graphics("images/transportation_risk.png")
```
...
# Open Science {background-color="#D9DBDB"}
...
`cholesterol ~ diet_fat + daily_exercise_hours + strain + diet_fat:daily_exercise`


```{r, echo = TRUE}
simulate_ancova <- function(n, alpha = 0.05, sd = 1){

# Two strains of mice
group <- factor(rep(1:2, each = n))

# dietary fat 
fat_amount <- runif(n, min = 20, max = 80)
# daily exercise
exercise_amount <- rnorm(n, mean = 30, sd = 4)

# coefficients

# Define coefficients
beta_fat = 0.08      # Fat increases cholesterol
beta_exercise = -.03  # Exercise decreases cholesterol
beta_interaction = -0.01  # Interaction term

strain_effect <- ifelse(group==1, 
                         rnorm(n, mean = 10), 
                         rnorm(n, mean = 5))
...
cholesterol <- rnorm(
  ...
  mean = 150+
  (fat_amount*beta_fat)+
  (exercise_amount*beta_exercise)+
  (beta_interaction*fat_amount*exercise_amount)+
  strain_effect,
  sd = sd)
 ...
 # Create dataframe
  ...
  model <- lm(cholesterol ~ group + 
                fat_amount + 
                exercise_amount + 
                fat_amount:exercise_amount, data = data)
  
  p_value <- summary(model)$coefficients[5,4]
...
monte_carlo_power <- function(n_sims=1000, n=30, alpha = 0.05, sd =1) {
  results <- replicate(n_sims, simulate_ancova(n, alpha))
  power <- mean(results)
  return(power)
}

# Run simulation
power_estimate <- monte_carlo_power(n_sims = 1000, n = 30, sd =1)
...
knitr::include_graphics("images/Ionnadis.png")
...
knitr::include_graphics("images/pub_perish.png")
...
knitr::include_graphics("images/manifesto_bad_science.png")
...
- There should be less focus/reward for people who are doing expensive science, and more focus/reward for people who are doing better science. 
...
- There is a huge computational skills gap amongst PhD students, postdocs, and teaching/research academics - without these skills, people simply cannot adopt open and reproducible research practices.


::: aside

https://bmcresnotes.biomedcentral.com/articles/10.1186/s13104-021-05883-3
...
knitr::include_graphics("images/error_3.png")
...
knitr::include_graphics("images/error_1.png")
...
knitr::include_graphics("images/error_2.png")

```

## You NEED Code

- You need to use open source analysis software such as R or Python.
...
knitr::include_graphics("images/github_front.png")

```

## You need to capture dependencies

- You need to capture the versions of your software packages. 
...
- Open data and open code are the future - remember to make your data and code available.
...
# Resources {background-color="#D9DBDB"}
